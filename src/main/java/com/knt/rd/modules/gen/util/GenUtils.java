 ...
 */
package com.knt.rd.modules.gen.util;
...
import com.knt.rd.common.mapper.JaxbMapper;
import com.knt.rd.common.utils.DateUtils;
import com.knt.rd.common.utils.FileUtils;
import com.knt.rd.common.utils.FreeMarkers;
import com.knt.rd.common.utils.StringUtils;
import com.knt.rd.modules.gen.entity.GenCategory;
import com.knt.rd.modules.gen.entity.GenConfig;
import com.knt.rd.modules.gen.entity.GenScheme;
...
public class GenUtils {
    ...
    for (GenTableColumn column : genTable.getColumnList()) {
      ...
      if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
        ...
        String[] ss = StringUtils
            .split(StringUtils.substringBetween(column.getJdbcType(), "(", ")"), ",");
  ...
  public static String getTemplatePath() {
    try {
      ...
      if (file != null) {
        return file.getAbsolutePath() + File.separator + StringUtils
            .replaceEach(GenUtils.class.getName(),
...
//		}
          ...
          if (list != null) {
            for (String s : list) {
              if (StringUtils.startsWith(s, GenCategory.CATEGORY_REF)) {
                templateList.addAll(
                    getTemplateList(config, StringUtils.replace(s, GenCategory.CATEGORY_REF, ""),
                        false));
              } else {
  ...
  public static Map<String, Object> getDataModel(GenScheme genScheme) {
    ...
    model.put("lastPackageName",
        StringUtils.substringAfterLast((String) model.get("packageName"), "."));
    ...
    model.put("functionAuthor",
        StringUtils.isNotBlank(genScheme.getFunctionAuthor()) ? genScheme.getFunctionAuthor()
            : UserUtils.getUser().getName());
    model.put("functionVersion", DateUtils.getDate());

    model.put("urlPrefix",
        model.get("moduleName") + (StringUtils.isNotBlank(genScheme.getSubModuleName())
            ? "/" + StringUtils.lowerCase(genScheme.getSubModuleName()) : "") + "/" + model
            .get("className"));
