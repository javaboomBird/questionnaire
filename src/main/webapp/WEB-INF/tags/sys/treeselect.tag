...
<%@ include file="/WEB-INF/views/include/childdomain.jsp" %>
...
<%@ attribute name="property" type="java.lang.String" required="false" description="" %>
<div class="input-append">
    <input id="${id}Id" name="${name}" class="${cssClass}" type="hidden" value="${value}"/>
    <input id="${id}IdName" name="${property}" class="${cssClass}" type="hidden" value="${value}"/>
...
$("#${id}Button, #${id}Name").click(function () {
  ...
  // 正常打开
  top.$.jBox.open("iframe:${ctx}/tag/treeselect?url=" + encodeURIComponent("${url}")
      + "&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300,
      420, {
        ...
        submit: function (v, h, f) {
          if (v == "ok") {
            ...
            for (var i = 0; i < nodes.length;
                i++) {//<c:if test="${checked && notAllowSelectParent}">
              if (nodes[i].isParent) {
