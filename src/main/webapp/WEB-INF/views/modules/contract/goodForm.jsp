...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/contract/good/form?id=${good.id}">合同商品<shiro:hasPermission name="contract:good:edit">${not empty good.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="contract:good:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="good" action="${ctx}/contract/good/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="type" class="input-xlarge ">
					...
					<form:options items="${fns:getDictList('ecm_good_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="control-group">
				...
				<div class="controls">
					<table id="contentTable_goodPic" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								<th class="hide"></th>
								<th>图片描述</th>
								...
								<th>文件类型</th>
								<th>图片路径</th>
								<shiro:hasPermission name="contract:good:edit"><th width="10">&nbsp;</th></shiro:hasPermission>
							</tr>
						</thead>
						<tbody id="goodPicList">
						</tbody>
						<shiro:hasPermission name="contract:good:edit"><tfoot>
							<tr><td colspan="6"><a href="javascript:" onclick="addRow('#goodPicList', goodPicRowIdx, goodPicTpl);goodPicRowIdx = goodPicRowIdx + 1;" class="btn">新增</a></td></tr>
					...
					<script type="text/javascript">
						var goodPicRowIdx = 0, goodPicTpl = $("#goodPicTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
						$(document).ready(function() {
		...
		<div class="form-actions">
			<shiro:hasPermission name="contract:good:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
