...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/projectmanager/projectStage/form?id=${projectStage.id}">项目阶段管理<shiro:hasPermission name="projectmanager:projectStage:edit">${not empty projectStage.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="projectmanager:projectStage:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="projectStage" action="${ctx}/projectmanager/projectStage/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="stageName" htmlEscape="false" maxlength="64" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="stageType" class="input-xlarge ">
					...
					<form:options items="${fns:getDictList('project_stage_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="manager"  property="managerId" name="manager.id" value="${projectStage.manager.id}" labelName="manager.name" labelValue="${projectStage.manager.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="progress" value="${projectStage.progress==null?0:projectStage.progress}" maxlength="10" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">有交接文档：</label>
			<div class="controls">
				<form:checkboxes path="hasDocument" items="${fns:getDictList('project_has_document')}" itemLabel="label" itemValue="value" htmlEscape="false" class=""/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="control-group">
				...
				<div class="controls">
					<table id="contentTable_projectDocument" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								<th class="hide"></th>
								<th>文档名称</th>
								...
								<%--<th>文档序号</th>--%>
								<th>备注</th>
								<shiro:hasPermission name="projectmanager:projectStage:edit"><th width="10">&nbsp;</th></shiro:hasPermission>
							</tr>
						</thead>
						<tbody id="projectDocumentList">
						</tbody>
						<shiro:hasPermission name="projectmanager:projectStage:edit"><tfoot>
							<tr><td colspan="8"><a href="javascript:" onclick="addRow('#projectDocumentList', projectDocumentRowIdx, projectDocumentTpl);projectDocumentRowIdx = projectDocumentRowIdx + 1;" class="btn">新增</a></td></tr>
					...
					<script type="text/javascript">
						var projectDocumentRowIdx = 0, projectDocumentTpl = $("#projectDocumentTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
						$(document).ready(function() {
		...
		<div class="form-actions">
			<shiro:hasPermission name="projectmanager:projectStage:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
