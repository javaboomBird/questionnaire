...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/group/sysGroup/form?id=${sysGroup.id}">分组管理<shiro:hasPermission name="group:sysGroup:edit">${not empty sysGroup.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="group:sysGroup:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="sysGroup" action="${ctx}/group/sysGroup/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
					...
					<table id="contentTable_sysUserGroup" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								...
								<th>用户</th>
								<th>备注</th>
								<shiro:hasPermission name="group:sysGroup:edit"><th width="10">&nbsp;</th></shiro:hasPermission>
							</tr>
						</thead>
						<tbody id="sysUserGroupList">
						</tbody>
						<shiro:hasPermission name="group:sysGroup:edit"><tfoot>
		...
		<div class="form-actions">
			<shiro:hasPermission name="group:sysGroup:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
