...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/engineering/engineeringOrderAccept/">工单受理列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="engineeringWorkOrder"
           action="${ctx}/engineering/engineeringOrderAccept/" method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" var="engineeringWorkOrder">
        <tr>
            <td>
                <a href="${ctx}/engineering/engineeringOrderAccept/form?id=${engineeringWorkOrder.id}">
            ...
            <td>
                    ${fns:getUserById(engineeringWorkOrder.assignorId).name}
            </td>
            <td>
                    ${fns:getDictLabel(engineeringWorkOrder.status, 'engineering_order_status', '')}
            </td>

            <shiro:hasPermission name="engineering:engineeringOrderAccept:edit">
                <td>
                    <c:choose>
                        <c:when test="${engineeringWorkOrder.status=='YFP'}">
                            <a href="${ctx}/engineering/engineeringOrderAccept/form?id=${engineeringWorkOrder.id}">受理</a>
                        </c:when>
                    </c:choose>
                </td>
            </shiro:hasPermission>
