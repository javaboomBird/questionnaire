...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/sysGroup/">分组管理列表</a></li>
		<li class="active"><a href="${ctx}/sys/sysGroup/form?id=${sysGroup.id}">分组管理<shiro:hasPermission name="sys:sysGroup:edit">${not empty sysGroup.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:sysGroup:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="sysGroup" action="${ctx}/sys/sysGroup/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
				...
				<div class="controls">
					<table id="contentTable_sysUserGroup" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								...
								<th>用户ID</th>
								<th>备注</th>
								<shiro:hasPermission name="sys:sysGroup:edit"><th width="10">&nbsp;</th></shiro:hasPermission>
							</tr>
						</thead>
						<tbody id="sysUserGroupList">
						</tbody>
						<shiro:hasPermission name="sys:sysGroup:edit"><tfoot>
							<tr><td colspan="4"><a href="javascript:" onclick="addRow('#sysUserGroupList', sysUserGroupRowIdx, sysUserGroupTpl);sysUserGroupRowIdx = sysUserGroupRowIdx + 1;" class="btn">新增</a></td></tr>
					...
					<script type="text/template" id="sysUserGroupTpl">//<!--
						<tr id="sysUserGroupList{{idx}}">
							...
							<shiro:hasPermission name="sys:sysGroup:edit"><td class="text-center" width="10">
								{{#delBtn}}<span class="close" onclick="delRow(this, '#sysUserGroupList{{idx}}')" title="删除">&times;</span>{{/delBtn}}
							</td></shiro:hasPermission>
						</tr>//-->
					</script>
					<script type="text/javascript">
						var sysUserGroupRowIdx = 0, sysUserGroupTpl = $("#sysUserGroupTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
						$(document).ready(function() {
		...
		<div class="form-actions">
			<shiro:hasPermission name="sys:sysGroup:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
